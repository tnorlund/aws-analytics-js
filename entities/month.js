...
class Month {
  /**
   * A month object.
   * @param {Object} details The details of the month.
   */
  constructor( {
    ...
    if ( !date_match )
      throw new Error( `Date must be given as <year>-<month>` )
  ...
  key() {
    return {
      ...
      'SK': variableToItemAttribute( `#MONTH#${ 
  ...
  gsi1() {
    return {
      ...
      'GSI1SK': variableToItemAttribute( `#MONTH#${ 
        ZeroPadNumber( this.year, 4 ) }-${ ZeroPadNumber( this.month, 2 ) }` )
    }
  }
  ...
  toItem() {
    return {
      ...this.key(),
      ...this.gsi1(),
      'Type': variableToItemAttribute( `month` ),
...
module.exports = { Month, monthFromItem }